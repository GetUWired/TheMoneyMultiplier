...
/*
Template Name: New Policy Weekly Report
*/
...
<?php 
		...
		function process_form_weekly_report() {
			$num = $_POST['pagenum'];
			$agentpw = $_POST['agentpw'];

		     if ($num == "9295" && $agentpw == 'NPWR1227'){
		...
		function create_weekly_report() {
			...
			echo '<p class="scroll">scroll &nbsp; &rarr;</p>
				<div class="table-wrapper" id="print-table">
					<table>
						<thead>
							<tr>
								<th><div style="width: 30px; white-space: normal;">Row</div></th>
			...
			$sql = 'SELECT * FROM NewPolicyWeeklyReport';
			// ORDER BY DateCreated DESC

			$stmt = $conn->prepare($sql);
			$stmt->execute();
			$weeklyReportInfo = $stmt->get_result();	

			//creates the rows in the report from the data in the table
			if ($weeklyReportInfo->num_rows > 0) {
			    // output data of each row		
			    $rowNum = 1;	    
			    while($row = $weeklyReportInfo->fetch_assoc()) {

			    	//creating rows
			    	$include_row = '

			    		<tr>
							<td><div style="width: 30px; white-space: normal;">' . $rowNum . '</div></td>
