...
//$app was set in conn.php
	...
	$deleteRows = "DELETE FROM NewPolicyWeeklyReport";
	// prepare
	...
	foreach ($allNewPolicies as $key => $value) {
		...
		$ownerFirstName = $value['Custom_OwnerNamePolicy1'];
		$ownerLastName = $value['Custom_Policy1OwnerLastName'];
		$policyAmt = $value['Custom_PolicyAmount'];

		$createdDateTime = $value['DateCreated'];
		$createdDateTrim = explode("T", $createdDateTime);
		$createdDate = $createdDateTrim[0];
		...
		$sql = "INSERT INTO NewPolicyWeeklyReport (Id, InsuredFirstName, InsuredLastName, OwnerFirstName, OwnerLastName, DateCreated, PolicyAmt, CommissionOwner1, CommissionOwner2, CommissionOwner3, CommissionOwner4, CommissionOwner5, CommissionOwner6, CommissionOwner7, CommissionOwner8, CommissionOwner9, CommissionOwner10) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

			if(!($stmt2 = $conn->prepare($sql))) {
				echo "insert fail: " . $conn->errno . " error " . $conn->error;
			}

			if(!$stmt2->bind_param("isssssissssssssss", $ISid, $firstName, $lastName, $ownerFirstName, $ownerLastName, $createdDate, $policyAmt, $new1, $new2, $new3, $new4, $new5, $new6, $new7, $new8, $new9, $new10)) {
